defmodule Parser do
  ...
  defp state_to_flags(
         <<_::1, broken_comm::1, no_comm::1, error_flag::1, _::1, battery::1, _::1, _::1>>
       ) do
    Map.merge(
      case error_flag do
        1 ->
          %{
            error: 1,
            error_message: "broken_communication:#{broken_comm} no_communication:#{no_comm}"
          }
  ...
  #
  # no error
  defp state_to_flags_type_2(<<0x00>>), do: %{}
  ...
  defp state_to_flags_type_2(<<_::6, application_errors::2>> = state) do
    case application_errors do
      0b01 ->
        %{
          error: 1,
          error_message: "application_error: application busy, state: #{inspect(state)}"
        }

      0b10 ->
        %{
          error: 1,
          error_message: "application_error: any application error, state: #{inspect(state)}"
        }

      # 0b011: reserved, 0b00: no error
      _ ->
        %{}
    end
  end
  ...
  defp additional_functions(
         <<no_usage::1, backflow::1, battery_low::1, _::1, broken_pipe::1, _::1,
           continous_flow::1, _::1>>
       ) do
    %{
  ...
  defp build_missing(
         %{meter_id: meter_id, medium: medium, volume: current_value},
         current_measured_at,
         meta
       ) do
    ...
    [
      ...
      },
      {
        ...
        %{
          ...
          _last_reading_map: %{
            [meter_id: 20_160_713, medium: :water, volume: :_] => %{
              measured_at: test_datetime("2019-01-01T04:11:11Z"),
              data: %{volume: 0.45}
            }
          }
        },
      ...
      },
      {
        ...
        %{
          ...
          _last_reading_map: %{
            [meter_id: 21_063_118, medium: :gas, volume: :_] => %{
