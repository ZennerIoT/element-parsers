defmodule Parser do
  ...
  # Status Message
  def parse(<<usage::little-32, battery::8, temperature::signed-8, _::signed-8, status::16>>, %{
        meta: %{frame_port: 24}
      }) do
    ...
    temp_detec_m =
      case temp_detec_m do
        0 -> "Off"
        1 -> "On"
      end

    temp_detec_s =
      case temp_detec_s do
        0 -> "Ok"
        1 -> "Alert"
      end
  ...
  def parse(<<0x00, serial::binary-4, firmware::24, reason::8, debug_info::binary>>, %{
        meta: %{frame_port: 99}
      }) do
    # need to reverse binary
    <<a, b, c, d>> = serial
  ...
  # Shutdown Message
  def parse(
        <<0x01, reason::8, usage::little-32, battery::8, temperature::signed-8, _::signed-8,
          status::16>>,
        %{meta: %{frame_port: 99}}
      ) do
    <<mode::8, state::8>> = <<status::16>>
  ...
  def tests() do
    [
      ...
      },
      {
        ...
        :parse_hex,
        "001601114C0006001004008300280140000000",
        %{meta: %{frame_port: 99}},
        %{
          message_type: "boot",
          serial_nr: "4C110116",
