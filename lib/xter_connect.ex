defmodule Parser do
  ...
  def parse(
        <<
          ...
          guard_time_night,
          keep_alive_value,
          pio0_event_threshold_day,
          pio0_event_threshold_night,
          pio1_event_threshold_day,
          pio1_event_threshold_night,
          pio2_event_threshold_day,
          pio2_event_threshold_night,
          pio3_event_threshold_day,
          pio3_event_threshold_night,
          night_start_hours,
          night_start_minutes,
          day_start_hours,
          day_start_minutes,
  ...
  defp parse_rest(<<hours, minutes, flags::binary-1, rest::binary>>) do
    ...
    # signed int in docs
    {rest, reading} = add_parse(reading, :uint12, vbat_present, rest, :battery_volt, 0.01)
    # signed int in docs
    {rest, reading} = add_parse(reading, :uint12, vext_present, rest, :external_volt, 0.01)
  ...
  defp add_parse(reading, type, 1, rest, key, _factor) do
    Logger.warn(
      "Missing #{inspect(type)} value in payload for #{inspect(key)} with binary: #{inspect(rest)}"
    )
