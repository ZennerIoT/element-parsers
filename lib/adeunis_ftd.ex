defmodule Parser do
  ...
  #   2021-05-27 [jb]: Formatted code. Added example payload from v2 firmware.
  #
  ...
  def parse_field(
        [{:has_temperature, true} | fields],
        <<temperature::signed-8, rest::binary>>,
        _acc
      ) do
  ...
  def parse_field(
        [{:has_gps, true} | fields],
        <<latitude::binary-4, longitude::binary-4, gps_quality::binary-1, rest::binary>>,
        acc
      ) do
    ...
    gps_reception_scale_name =
      case gps_reception_scale do
        1 -> :good
        2 -> :average
        3 -> :poor
        _ -> :unknown
  ...
  def parse_field(
        [{:has_rssi_and_snr, true} | fields],
        <<rssi::8, snr::signed-8, rest::binary>>,
        acc
      ) do
    acc =
      Enum.concat(
  ...
  def parse_status(
        <<has_temperature::1, trigger_accelerometer::1, trigger_push_button::1, has_gps::1,
          has_up_fcnt::1, has_down_fcnt::1, has_battery_level::1, has_rssi_and_snr::1>>
      ) do
    {[
    ...
    [
      {
        ...
        "CF1E6C390EA57102",
        %{
          _comment: "Example from v1 firmware"
        },
        %{
      ...
      },
      {
        ...
        %{
          _comment: "Example from v2 firmware"
        },
        {
          %{
            battery_voltage: 4041,
            down_fcnt: 32,
            gps_reception_scale: 2,
            gps_reception_scale_name: :average,
            gps_satellites: 7,
            gw_rssi: nil,
            latitude: 45.26615,
            longitude: 5.575,
            ...
            snr: 7,
            temperature: 27,
            trigger_accelerometer: false,
            trigger_push_button: true,
            up_fcnt: 32
          },
          [location: {5.575, 45.26615}]
        }
      }
    ]
  end
