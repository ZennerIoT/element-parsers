defmodule Parser do
  ...
  def add_bosch_parking_format(fields) do
    ...
    |> case do
      %{parking_status: 1} = fields ->
        Map.merge(fields, %{message_type: :parking_status, p_state: :occupied})

      %{parking_status: 0} = fields ->
        Map.merge(fields, %{message_type: :parking_status, p_state: :free})
  ...
  defp add_vehicle_count(data, count) when count > 0x80,
    do: Map.merge(data, %{event: :reboot_or_recalibration})
