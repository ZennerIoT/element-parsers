defmodule Parser do
  ...
  # Real-Time Data
  def parse(
        <<0x72, co2_level, temperature, humidity, iaq_global::3, iaq_src::4, iaq_co2::3,
          iaq_dry::3, iaq_mould::3, iaq_dm::3, hci::2, frame_index::3>>,
        _meta
      ) do
    %{
    ...
    |> add_from_mapping(
      :iaq_global,
      iaq_global,
      mapping(~w(excellent good fair poor bad reserved reserved error)),
      :unknown
    )
    |> add_from_mapping(
      :iaq_src,
      iaq_src,
      mapping(~w(all drought_index mold_index mite_index co co2)),
      :unknown
    )
    |> add_from_mapping(
      :iaq_co2,
  ...
  # Product Information
  def parse(
        <<0x73, battery_level::2, hw_status::1, frame_index::3, _unused::2, product_activation,
          co2_autocalibration, year::6, month::4, day::5, hour::5, minute::6, _::6>>,
        _meta
      ) do
    %{
      product_type: :insafe_carbon_lora,
    ...
    |> add_from_mapping(
      :battery_level,
      battery_level,
      mapping(~w(high medium low critical)),
      :unknown
    )
  ...
  # Message datalog
  def parse(
        <<0x75, n2::3-binary, n1::3-binary, n0::3-binary, time_between::4, frame_index::3, _::1,
          _::binary>>,
        %{transceived_at: transceived_at}
      ) do
    template =
  ...
  def parse(
        <<0x78, co2_threshold1, co2_threshold2, sp1_start1::6, sp1_start2::6, sp2_start1::6,
          sp2_start2::6, sp1::binary-1, sp2::binary-1, altitude>>,
        _meta
      ) do
    <<sp1_active::1, sp1_mo::1, sp1_tu::1, sp1_we::1, sp1_th::1, sp1_fr::1, sp1_sa::1, sp1_su::1>> =
      sp1

    <<sp2_active::1, sp2_mo::1, sp2_tu::1, sp2_we::1, sp2_th::1, sp2_fr::1, sp2_sa::1, sp2_su::1>> =
      sp2
  ...
  def parse(
        <<0x79, flags::binary-1, meas_period, datalog_factor, temp_alert1, temp_alert2,
          temp_delta, humi_delta, co2_delta, keepalive, sw_version>>,
        _meta
      ) do
  ...
  defp add_range_with_scale(row, column, value, _range, _scale, error_value, error_code)
       when value == error_value do
    Map.merge(row, %{
      "#{column}_error" => error_code
    })
  end

  defp add_range_with_scale(
         row,
         column,
         value,
         %Range{first: 0, last: range2},
         %Range{first: 0, last: scale2},
         _error_value,
         _error_code
       ) do
    step = scale2 / range2
  ...
  def tests() do
    [
      ...
      {:parse_hex, "75 1a674d 186a4c 196f4a 6e",
       %{meta: %{frame_port: 1}, transceived_at: test_datetime("2020-02-24T12:00:00Z")},
       [
         {%{
            co2_level: 500.0,
            frame_index: 7,
      ...
      # Keep alive
      {:parse_hex, "7A", %{meta: %{frame_port: 1}},
       %{message_type: :keep_alive, product_type: :insafe_carbon_lora}}
    ]
  end
