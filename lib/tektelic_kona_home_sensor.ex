defmodule Parser do
  ...
  def parse_frames(<<0x00, 0xFF, battery::16-signed, rest::binary>>, frames) do
    # Conversion 10mV in V
    battery_volt = battery * 10 / 1000
  ...
  def parse_frames(<<0x04, 0x68, relative_humidity, rest::binary>>, frames) do
    # Conversion from 0.5% to %
    relative_humidity_percent = relative_humidity / 2
  ...
  def parse_frames(<<0x05, 0x02, impact::16-signed, rest::binary>>, frames) do
    # Conversion from 1 milli-g to g
    impact_g = impact / 1000
  ...
  def parse_frames(<<0x07, 0x71, accelero::binary-6, rest::binary>>, frames) do
    ...
    parse_frames(rest, [
      # g
      {:acceleration_x, x / 100},
      # g
      {:acceleration_y, y / 100},
      # g
      {:acceleration_z, z / 100}
      | frames
    ])
  end
  ...
  def tests() do
    [
      # Real payload
      {:parse_hex, "036700DE04686A00FF0133", %{meta: %{frame_port: 10}},
       %{battery: 3.07, relative_humidity: 53.0, temperature: 22.2}},

      # Examples from docs
      {:parse_hex, "03 67 00 0A 04 68 28", %{meta: %{frame_port: 10}},
       %{relative_humidity: 20.0, temperature: 1.0}},
      {:parse_hex, "04 68 14 01 00 FF 08 04 00 05", %{meta: %{frame_port: 10}},
       %{reed_switch: 0, reed_switch_counter: 5, relative_humidity: 10.0}},
      {:parse_hex, "04 68 2A 03 67 FF FF 00 FF 01 2C", %{meta: %{frame_port: 10}},
       %{battery: 3.0, relative_humidity: 21.0, temperature: -0.1}},
      ...
      {:parse_hex, "0D 04 00 02 06 00 FF", %{meta: %{frame_port: 10}},
       %{break_in: 1, motion_count: 2}}
    ]
  end
end
