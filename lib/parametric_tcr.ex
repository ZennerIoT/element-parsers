defmodule Parser do
  ...
  def parse(
        <<
          ...
          0xBE,
          # Device Family, always 0x01 for TCR Devices
          0x02,
          # Payload Version, always 0x01 for V3 Payloads
          0x01,
  ...
  def parse(
      ...
      when payload_version in [0x01, 0x02] do
    <<f1, f2, f3>> = fw_version

    device_type =
      case payload_version do
        0x01 ->
          Map.get(%{0 => :tcr, 1 => :tcr_s}, device_type, device_type)

        0x02 ->
          Map.get(
            %{0 => :tcr_ls, 1 => :tcr_lss, 2 => :tcr_hs, 3 => :tcr_hss},
            device_type,
            device_type
          )
  ...
  defp calc_totals(row, direction) do
    total_count =
      Map.get(row, "#{direction}_count_class0", 0) + Map.get(row, "#{direction}_count_class1", 0) +
        Map.get(row, "#{direction}_count_class2", 0) +
        Map.get(row, "#{direction}_count_class3", 0)

    total_avg =
      Map.get(row, "#{direction}_count_class0", 0) * Map.get(row, "#{direction}_avg_class0", 0) +
        Map.get(row, "#{direction}_count_class1", 0) * Map.get(row, "#{direction}_avg_class1", 0) +
        Map.get(row, "#{direction}_count_class2", 0) * Map.get(row, "#{direction}_avg_class2", 0) +
        Map.get(row, "#{direction}_count_class3", 0) * Map.get(row, "#{direction}_avg_class3", 0)
