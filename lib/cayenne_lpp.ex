defmodule Parser do
  ...
  defp _parse(<<channel, 0x02, data::16-signed, rest::binary>>, list, opts) do
    data = %{
      # 0.01 Signed
      analog_input: round_float(data * 0.01),
      channel: channel
    }
  ...
  defp _parse(<<channel, 0x03, data::16-signed, rest::binary>>, list, opts) do
    data = %{
      # 0.01 Signed
      analog_output: round_float(data * 0.01),
      channel: channel
    }
  ...
  defp _parse(<<channel, 0x67, data::16-signed, rest::binary>>, list, opts) do
    data = %{
      # 0.1 °C Signed MSB
      temperature: round_float(data * 0.1),
      channel: channel
    }
  ...
  # Accelerometer
  defp _parse(
         <<channel, 0x71, data_x::16-signed, data_y::16-signed, data_z::16-signed, rest::binary>>,
         list,
         opts
       ) do
    data = %{
      # 0.001 G Signed MSB per axis
      accelerometer_x: round_float(data_x * 0.001),
  ...
  defp _parse(<<channel, 0x73, data::16, rest::binary>>, list, opts) do
    data = %{
      ...
      barometer: round_float(data * 0.1),
      channel: channel
    }
  ...
  # Gyrometer
  defp _parse(
         <<channel, 0x86, data_x::16-signed, data_y::16-signed, data_z::16-signed, rest::binary>>,
         list,
         opts
       ) do
    data = %{
      # 0.01 °/s Signed MSB per axis
      gyrometer_x: round_float(data_x * 0.01),
  ...
  # GPS Location
  defp _parse(
         <<channel, 0x88, data_x::24-signed, data_y::24-signed, data_z::24-signed, rest::binary>>,
         list,
         opts
       ) do
  ...
  def tests() do
    [
      ...
      {:parse_hex, "03 67 01 10 05 67 00 FF", %{},
       [
         {%{channel: 5, temperature: 25.5}, []},
         {%{channel: 3, temperature: 27.2}, []}
       ]},
      ...
      {:parse_hex, "066700E607684A038807D3070118C10055D2040302580503058C", %{},
       [
         {%{analog_output: 14.2, channel: 5}, [location: {7.1873000000000005, 51.2775}]},
      ...
      {:parse_hex, "066700E6076848038807D3070118C1004D4E040301FE05030456", %{},
       [
         {%{analog_output: 11.1, channel: 5}, [location: {7.1873000000000005, 51.2775}]},
         {%{analog_output: 5.1, channel: 4}, [location: {7.1873000000000005, 51.2775}]},
      ...
      {:parse_hex, "066700E607684A038807D3080118C10053A204030276050304CE", %{},
       [
         {%{analog_output: 12.3, channel: 5}, [location: {7.1873000000000005, 51.2776}]},
