defmodule Parser do
  ...
  def parse(
        <<vbat::little-16, temp::little-16-signed, _res, data::binary>>,
        %{meta: %{frame_port: 2}} = meta
      ) do
    min_width = get_min_width(meta)
    min_amplitude = get_min_amplitude(meta)

    readings =
      for <<dist::little-32, tof_us::little-16, width, amplitude <- data>> do
  ...
  def tests() do
    [
      {:parse_hex, "5B0DE6000158080000113181DB", %{meta: %{frame_port: 2}},
       %{distance1_m: 2.136, distance1_mm: 2136, temperature: 23.0}},
      {:parse_hex, "5F0DE90001930100009709EFF5", %{meta: %{frame_port: 2}},
       %{distance1_m: 0.403, distance1_mm: 403, temperature: 23.3}},
      {:parse_hex, "2E0AF000017C0100001009A5EB", %{meta: %{frame_port: 2}},
       %{distance1_m: 0.38, distance1_mm: 380, temperature: 24.0}}
    ]
  end
end
