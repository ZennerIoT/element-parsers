defmodule Parser do
  ...
  # Version 2 of sensor
  def parse(
        <<0x43, _status::8, internal_identifier::8, internal_value::signed-16,
          external_identifier::8, external_value::signed-16>>,
        _meta
      ) do
  ...
  defp parse_timestamp(%{timestamp: 1} = row, code, payload)
       when code in [
              # keep alive
              0x30,
              # periodic data
  ...
  defp parse_status(
         row,
         <<frame_counter::3, channels::1, configuration_problem::1, timestamp::1, low_bat::1,
           config::1>>
       ) do
    %{
    ...
    |> Enum.into(%{
      ...
      channels: channels + 1,
      # If 1, a timestamp is appended to some payloads
      timestamp: timestamp
    })
    |> Map.merge(row)
  end
  ...
  defp parse_body(row, 0x10, <<s300::16, s301::16, s320::16, s321::16, s323::8>>, _meta) do
    Map.merge(row, %{
      ...
      transmission_period_keepalive: s300 * 10,
      # Number of backups (history logs) to be done before sending a frame (thus defining the sending period). The value 0 is equivalent to disabling the periodic mode.
      transmission_period_data: s301,
      # Number of readings to be performed before saving in the history logs. The value 1 is equivalent to 1 backup per reading
      history_period: s320,
      # Seconds between measuring temperature
      sampling_period: s321 * 2,
      # Number of samples to be repeated in the next frame (0..23)
      redundancy: s323
    })
  end
  ...
  defp parse_body(
         row,
         0x20,
         <<_rfu1::2, class_t::1, _rfu2::2, dc::1, _rfu3::1, adr::1, _rfu4::7, act::1>>,
         _meta
       ) do
    activation =
      case act do
  ...
  defp parse_body(
         %{channels: 2} = row,
         0x57,
         <<t1::binary-2, t2::binary-2, _::binary>> = body,
         %{transceived_at: transceived_at} = meta
       ) do
    row = Map.merge(row, %{frame_type: :data})
  ...
  defp parse_body(
         %{channels: 2} = row,
         0x58,
         <<alarm1, temp1::binary-2, alarm2, temp2::binary-2>>,
         _meta
       ) do
    row
    |> Map.merge(%{
    ...
    Enum.map(times, fn {time, index} ->
      row =
        case time do
          {time1, time2} ->
            reading_template
            |> add_valid_temp(time1, :temp_ch1)
            |> add_valid_temp(time2, :temp_ch2)
