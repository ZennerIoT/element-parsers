defmodule Parser do
  ...
  ### Status Message ###
  def parse(
        <<unix_timestamp::32-little, status, rssi::signed, profiles::binary>>,
        %{meta: %{frame_port: 24}} = _meta
      ) do
    <<
      relay_2::1,
      fw_error::1,
  ...
  def parse(
        <<0x06, sunrise_offset::signed, sunset_offset::signed, latitude::little-16,
          longitude::little-16>>,
        %{meta: %{frame_port: 50}}
      ) do
    %{
  ...
  def parse(<<0x08, profile_id, profile_seq, addr, days_active, rest::binary>>, %{
        meta: %{frame_port: 50}
      }) do
    map = %{
      type: "config",
  ...
  def parse(
        <<0x00, serial::little-32, fw::24, timestamp::little-32, hw, opt, _rest::binary>>,
        %{meta: %{frame_port: 99}} = _meta
      ) do
  ...
  defp profile_addr_to_dali_addr(<<1::1, 0::1, 0::1, dali_group_addr::4, 0::1>>),
    do: "Group address #{dali_group_addr}"

  defp profile_addr_to_dali_addr(<<0::1, dali_single_addr::6, 0::1>>),
    do: "Single device #{dali_single_addr}"
  ...
  defp read_usage_messages(<<addr, reported_fields, rest::binary>>) do
    <<
      ...
      system_voltage::1,
      # uint8, divide by 100
      power_factor_instant::1,
      # uint16, W
      load_side_energy_instant::1,
      # uint32, Wh
      load_side_energy_total::1,
      # uint16, W
      active_energy_instant::1,
      # uint32, Wh
      active_energy_total::1
    >> = <<reported_fields>>
  ...
  defp add_load_side_energy_instant(
         {map, <<load_side_energy_instant::little-16, rest::binary>>},
         1
       ) do
    {
  ...
  def tests() do
    [
      ...
      {
        :parse_hex,
        "28E29B59018E0416FE1E32022AFEE132",
        %{meta: %{frame_port: 24}},
        %{
          "profile_0_id" => 4,
          "profile_0_seq" => 22,
          ...
          relay_2: :off,
          rssi: -114,
          timestamp_unix: 1_503_388_200,
          timestamp: DateTime.from_unix!(1_503_388_200),
          type: "status"
        }
      },
      ...
      {
        :parse_hex,
        "04030000000000000603151400000000",
        %{meta: %{frame_port: 25}},
        [
          %{
      ...
      {
        :parse_hex,
        "011F4E04000006004D04000007002A",
        %{meta: %{frame_port: 25}},
        [
          %{
      ...
      # set device time
      {
        ...
        %{
          type: "config",
          timestamp: DateTime.from_unix!(1_556_884_344),
          timestamp_unix: 1_556_884_344
        }
      },
      ...
      {
        :parse_hex,
        "0348A1FE48A2A848A3FE48A4FE48A507",
        %{meta: %{frame_port: 60}},
        [
          %{
            type: "command",
      ...
      # boot (sample from docs)
      {
        ...
        %{
          ...
          serial: "471000FF",
          firmware: "0.4.5",
          timestamp: DateTime.from_unix!(1_497_437_379),
          timestamp_unix: 1_497_437_379,
          hw_setup: 5,
          opt: 0
        }
      },
