defmodule Parser do
  ...
  def parse_parts(
        result,
        <<_history::1, 3::7, _spare::6, low_alarm::1, high_alarm::1, rest::binary>>
      ) do
    result
  ...
  def parse_parts(
        result,
        <<_history::1, 5::7, _spare::6, low_alarm::1, high_alarm::1, rest::binary>>
      ) do
    result
  ...
  def parse_parts(result, <<_history::1, 7::7, ambient::16, rest::binary>>) do
    result
    # Lux
    |> Map.merge(%{ambient_light: ambient})
    |> parse_parts(rest)
  end
  ...
  def parse_parts(result, <<_history::1, 19::7, proximity::16, rest::binary>>) do
    result
    # dont know
    |> Map.merge(%{ir_proximity: proximity, ir_power: :high})
    |> parse_parts(rest)
  end
  ...
  def parse_parts(result, <<_history::1, 20::7, proximity::16, rest::binary>>) do
    result
    # dont know
    |> Map.merge(%{ir_proximity: proximity, ir_power: :low})
    |> parse_parts(rest)
  end
  ...
  def parse_parts(
        result,
        <<_history::1, 110::7, build_mod::1, build_id::31, status_info::32, rest::binary>>
      ) do
    result
    |> Map.merge(%{
  ...
  def parse_parts(result, <<_history::1, 112::7, cap_val::16, rest::binary>>) do
    result
    # Raw value from capacitance sensor
    |> Map.merge(%{flood_capacitance: cap_val})
    |> parse_parts(rest)
  end
  ...
  def parse_parts(result, <<_history::1, 113::7, cap_val::16, rest::binary>>) do
    result
    # Raw value from capacitance sensor
    |> Map.merge(%{pad_capacitance: cap_val})
    |> parse_parts(rest)
  end
  ...
  def parse_parts(result, <<_history::1, 114::7, cap_val::16, rest::binary>>) do
    result
    # Raw value from capacitance sensor
    |> Map.merge(%{end_capacitance: cap_val})
    |> parse_parts(rest)
  end
  ...
  def tests() do
    [
      ...
      # Real payloads
      # door closed
      {:parse_hex, "FFFF 0B01", %{meta: %{frame_port: 1}}, %{tamper: 1}},
      # door open
      {:parse_hex, "FFFF 0B00", %{meta: %{frame_port: 1}}, %{tamper: 0}},
      # door closed
      {:parse_hex, "FFFF 0901", %{meta: %{frame_port: 1}}, %{door_closed: 1}},
      # door open
      {:parse_hex, "FFFF 0900", %{meta: %{frame_port: 1}}, %{door_closed: 0}},
