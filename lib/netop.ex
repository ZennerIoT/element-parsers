defmodule Parser do
  ...
  # Protocol version 1
  def parse(<<_rfu::5, 1::3, serial_number::32, data_block_plus_checksum::binary>>, %{
        meta: %{frame_port: 22}
      }) do
  ...
  defp parse_data(
         <<_contains_timestamp::1, _rfu1::1, _slot_number::3, 1::1, device_information::9,
           _rfu2::1, sensor_board_type::binary-1>>,
         data
       ) do
    Logger.error(
      "Potentially invalid data. Device_information: #{inspect(device_information)} with sensor board type: #{
        inspect(sensor_board_type(sensor_board_type))
      } and data #{inspect(data)}"
    )
  ...
  # device information
  defp parse_data(
         <<contains_timestamp::1, _rfu1::1, slot_number::3, might_have_error::1,
           device_information::9, _rfu2::1, 0>>,
         data
       ) do
  ...
  defp parse_data(
         <<contains_timestamp::1, _rfu1::1, slot_number::3, might_have_error::1,
           sensor_function::9, _rfu2::1, sensor_board_type::binary-1>>,
         data
       ) do
    with {:ok, timestamp, rest} <- parse_time_stamp(contains_timestamp, data),
  ...
  # 7.3 plugged slots
  defp parse_device_information(
         0x002,
         <<slot_8::1, slot_7::1, slot_6::1, slot_5::1, slot_4::1, slot_3::1, slot_2::1, slot_1::1,
           rest::binary>>
       ) do
    {
      :ok,
  ...
  defp battery_level(battery) when 0 <= battery and battery <= 100,
    do: {:ok, :not_charging, battery}

  defp battery_level(battery) when 100 < battery and battery <= 200,
    do: {:ok, :charging, battery - 100}
  ...
  def tests() do
    [
      ...
      {:parse_hex,
       "011000006400080044030002000400040001000C00000D000A00000B08060000610061080C00000A080A00000AF8",
       %{meta: %{frame_port: 22}},
       [
         %{
           might_have_an_error: 0,
           serial_number: 268_435_556,
           slot_number: 1,
           sw_version: "v1.0",
           type: :device_information
         },
      ...
      {
        :parse_hex,
        "01100000640000000000000300000000000000000000000000000000000000000087",
        %{meta: %{frame_port: 22}},
        [
          %{
            might_have_an_error: 0,
      ...
      # 5.16 Door sensor without device information
      {
        ...
        [
          %{
            ...
            serial_number: 268_435_556,
            slot_number: 1,
            timestamp: DateTime.from_unix!(946_687_718),
            timestamp_linux: 946_687_718,
            type: :door_sensor
          }
        ]
      },
      ...
      {
        :parse_hex,
        "011000006400000000001A3388201339A6D054007F",
        %{meta: %{frame_port: 22}},
        [
          %{
            door_status: 0,
            ...
            serial_number: 268_435_556,
            slot_number: 1,
            timestamp: DateTime.from_unix!(967_233_620),
            timestamp_linux: 967_233_620,
            type: :door_sensor
          },
          %{
            might_have_an_error: 0,
      ...
      {
        :parse_hex,
        "01100006C388280B0000000000005060BA",
        %{meta: %{frame_port: 22}},
        [
          %{
