defmodule Parser do
  ...
  def parse(<<0::2, 0::1, 0::1, 0::1, type::3, frame::binary>>, meta) do
    case type do
      ...
      # Ignored: FrameTypeRawSerial and FrameTypeIec1107
      _ ->
  ...
  def parse(payload, meta) do
    Logger.info(
      "Can not parse frame with payload: #{inspect(payload)} on frame port: #{
  ...
  def parse_meter_reading_message(
        <<1::2, 2::3, qualifier::3, meter_id::32-little, registers::binary>>,
        meta
      ) do
    medium = 2
    ...
    [reading] ++
      build_missing(reading, :register_value, meta, %{
        medium: medium,
        qualifier: qualifier,
        register_index: 1
      })
  ...
  def parse_meter_reading_message(
        <<qualifier::8, 2::2, has_timestamp::1, is_compressed::1, medium::4, rest::binary>>,
        meta
      ) do
    case {{medium, qualifier, has_timestamp, is_compressed}, rest} do
      {{2, 4, 1, 0},
       <<meter_id::32-little, timestamp::32-little, register_value::32-little,
         register2_value::32-little>>} ->
        ...
        [reading] ++
          ...
          build_missing(reading, :register2_value, meta, %{
            medium: medium,
            qualifier: qualifier,
            register_index: 2
          })
      ...
      {header, binary} ->
        Logger.info(
          "Not creating meter reading because not matching header #{inspect(header)} and reading_data #{
  ...
  defp build_missing(%{} = current_data, register_field, meta, %{
         medium: medium,
         qualifier: qualifier,
  ...
  defp create_latest_meter_readings(
         medium,
         qualifier,
         meter_id,
         meta,
         <<timestamp::32-little, register_value::32-little, _other_register_values::binary>>
       ) do
    value = round_as_float(register_value / 100)
    ...
    reading =
      ...
      # From device, can be wrong if device clock is wrong
      |> Map.put(:timestamp_unix, timestamp)
      |> Map.put(:timestamp, DateTime.from_unix!(timestamp))
      |> add_power_from_last_reading(meta, :register_value, :power)
      |> add_obis(medium, qualifier, 1, value)
  ...
  def parse_status_message(
        <<reset_reason::3, node_type::2, session_info::3, firmware_id::binary-4,
          uptime_ms::32-little, time_s::32-little, last_downlink_ms::32-little, rssi::16-little,
          snr::8, frame_type::8, is_ack::8, connected_devices::8>>
      ) do
    %{
      type: "status",
  ...
  def add_power_from_last_reading(
        data,
        %{transceived_at: transceived_at} = meta,
        register_field,
        power_field
      ) do
    ...
    case {config(:add_power_from_last_reading, meta), is_nil(field_value)} do
      ...
      # Not activated or missing field
      _ ->
        data
    end
  end
  ...
  # a-plus
  defp obis_medium_qualifier(2, 1), do: ["1-0:1.8.0"]
  # a-plus-a-minus
  defp obis_medium_qualifier(2, 4), do: ["1-0:1.8.0", "1-0:2.8.0"]
  # a-minus
  defp obis_medium_qualifier(2, 5), do: ["1-0:2.8.0"]
  ...
  def tests() do
    last_reading_register_value = %{
      measured_at: test_datetime("2019-01-01T11:34:56Z"),
      data: %{"register_value" => 0.09}
    }

    last_reading_register2_value = %{
      measured_at: test_datetime("2019-01-01T11:34:56Z"),
      data: %{"register2_value" => 0.0}
    }
    ...
    [
      ...
      },
      {
        ...
        :parse_hex,
        "0169008178E17F98F44A042D7B4F4B000000000000000001",
        ...
        %{
          ...
          session_info: "joined",
          snr: 0,
          time_s: 1_333_472_516,
          type: "status",
          uptime_ms: 1_257_543_807
        }
      },
      {
        ...
        %{
          ...
          _last_reading_map: %{
            [register_value: :_] => %{
              measured_at: test_datetime("2019-01-01T12:11:11Z"),
              data: %{"register_value" => 1.23}
            }
          }
        },
        [
      ...
      # 00 04A2 0FE46503 AEA64F4B 83010000 00000000
      {
        ...
        :parse_hex,
        "0004A20FE4650327AA4F4B8301000000000000",
        %{meta: %{frame_port: 8}, transceived_at: test_datetime("2019-01-01T12:34:56Z")},
        [
          %{
            :header_version => 2,
            ...
            :register2_value => 0.0,
            :register_value => 3.87,
            :timestamp => DateTime.from_unix!(1_263_512_103),
            :timestamp_unix => 1_263_512_103,
            :type => "meter_reading",
            "1-0:1.8.0" => 3.87,
            :obis_value => 3.87,
            "1-0:2.8.0" => 0.0
      ...
      },
      {
        ...
        :parse_hex,
        "0004A20FE46503AEA64F4B8301000000000000",
        %{
          meta: %{frame_port: 8},
          transceived_at: test_datetime("2019-01-01T12:34:56Z")
        },
        [
          %{
            ...
            :register2_value => 0.0,
            :register_value => 3.87,
            :timestamp => DateTime.from_unix!(1_263_511_214),
            :timestamp_unix => 1_263_511_214,
            :type => "meter_reading",
            "1-0:1.8.0" => 3.87,
            :obis_value => 3.87,
            "1-0:2.8.0" => 0.0
      ...
      },
      {
        ...
        :parse_hex,
        "0001A27D29370046237B4BCF0100002E227B4BCF01000062217B4BCF010000",
        %{meta: %{frame_port: 8}, transceived_at: test_datetime("2019-01-01T12:34:56Z")},
        [
          %{
            :header_version => 2,
            ...
            :register_value => 4.63,
            :obis_value => 4.63,
            :timestamp => DateTime.from_unix!(1_266_361_158),
            :timestamp_unix => 1_266_361_158,
            :type => "meter_reading",
            "1-0:1.8.0" => 4.63
          }
        ]
      ...
      },
      {
        ...
        :parse_hex,
        "0001A277293700F7287A4B1A0400006B287A4B19040000B0277A4B1904000024277A4B19040000",
        %{
          meta: %{frame_port: 8},
          transceived_at: test_datetime("2019-01-01T12:34:56Z")
        },
        [
          %{
            ...
            :register_value => 10.5,
            :obis_value => 10.5,
            :timestamp => DateTime.from_unix!(1_266_297_079),
            :timestamp_unix => 1_266_297_079,
            :type => "meter_reading",
            "1-0:1.8.0" => 10.5
          }
        ]
