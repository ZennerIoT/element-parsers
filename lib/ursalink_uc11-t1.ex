defmodule Parser do
  ...
  def parse_part(
        <<0xFF, 0x0D, mode, lower_tres::little-16-signed, upper_tres::little-16-signed,
          temp::little-16-signed, rest::binary>>,
        row
      ) do
    rest
    |> parse_part(row)
  ...
  def parse_part(rest, row),
    do: Map.merge(row, %{error: :invalid_payload, payload_rest: Base.encode16(rest)})
