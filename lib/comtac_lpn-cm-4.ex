defmodule Parser do
  ...
  # Seconds
  defp default_measurement_rate(), do: 3840

  # Port 3: Application Data
  def parse(<<1, status::binary-2, battery_voltage, rest::binary>>, %{
        meta: %{frame_port: 3},
  ...
  # Port 101: Info Data
  def parse(
        <<1, status::binary-2, battery_voltage, app_main_version, app_minor_version,
          _rest::binary>>,
        %{meta: %{frame_port: 101}}
      ) do
    %{
  ...
  defp parse_payload(acc, <<
         04,
         # According to documentation, these should be "little", but it is not.
         temp0::16-signed,
         humi0::8,
         temp1::16-signed,
         humi1::8,
  ...
  defp parse_payload(acc, <<rest::binary>>),
    do: Map.merge(acc, %{error: "unknown_binary_part: #{inspect(rest)}"})
  ...
  def tests() do
    [
      {:parse_hex, "011202C804096A22096A21096622096921095C21095721095721094820",
       %{meta: %{frame_port: 3}, transceived_at: test_datetime("2020-02-24T12:00:00Z")},
       [
         {%{
            acc: 0,
            battery: 100,
